...
class NearestNeighborClassifier:
    ...
    def compute_data_statistics(cls, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ## I googled to understand the dim=0 and the keepdim=True
        ## did this because I had just this "return (torch.mean(x), torch.std(x))" and was getting stuck.
    ...
    def get_nearest_neighbor(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # raise NotImplementedError
        ## I used chat gpt here to realize I had to input normalize the self.data aswell
        x = self.input_normalization(x)
        temp = self.input_normalization(self.data)
        idx = ((temp - x).pow(2).sum(dim=1))
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        ## technically used chat gpt here too since I learned about normalizing self.data in the previous exercise
        x = self.input_normalization(x)
        temp = self.input_normalization(self.data)
        idx = ((temp - x).pow(2).sum(dim=1))
        _, idx = idx.topk(k, largest=False)
        return self.data[idx], self.label[idx]

    def knn_regression(self, x: torch.Tensor, k: int) -> torch.Tensor:
        ## technically used chat gpt here too since I learned about normalizing self.data in the previous exercise
        ## also used it to return the correct thing .float() etc that final return statement 
        x = self.input_normalization(x)
        temp = self.input_normalization(self.data)
        idx = ((temp - x).pow(2).sum(dim=1))
        _, idx = idx.topk(k, largest=False)
        return torch.mean(torch.tensor(self.label[idx]).float())
        """
